...
def rdf_to_dot(g, stream, opts={}):
    ...
    stream.write('digraph { rankdir=LR \n node [ fontname="DejaVu Sans" fontcolor="#201D7F"] ; \n')
    # test
    ...
    for u, n in nodes.items():
        ...
        opstr = (
            "%s [ shape=none, color=%s label=< <table color='#201D7F'"
            + " cellborder='0' cellspacing='0' border='1'><tr>"
            + "<td colspan='2' bgcolor='#eeeeee'><B>%s</B></td></tr><tr>"
            + "<td href='%s' bgcolor='#eeeeee' colspan='2'>"
            + "<font point-size='10' color='#24A2D5'>%s</font></td>"
            + "</tr>%s</table> > ] \n"
        )
        stream.write(
            opstr
...
def main():
    rdflib.extras.cmdlineutils.main(rdf_to_dot, _help)
